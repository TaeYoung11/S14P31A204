...
export function useConnections() {
  ...
  const upsertConnection = useCallback((from: string, to: string, type: ConnectionStyle) => {
   ...
   */
  const openModal = useCallback((selectedId: string | null, previousSelectedId: string | null) => {
   ...
   */
  const openModalWithPair = useCallback((from: string, to: string, existingStyle?: ConnectionStyle) => {
  ...
  }, [connectionPair, selectedStyle, upsertConnection])

  /** 특정 버블과 연결된 모든 연결선 삭제 (버블 삭제 시 호출) */
  const removeConnectionsForBubble = useCallback((id: string) => {
