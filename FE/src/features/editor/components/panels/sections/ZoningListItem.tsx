...
/**
 * 조닝 한 행(Row)을 렌더링한다.
 * - 자동 조닝: 삭제 버튼 없이 수정 진입만 허용
 * - 수동 조닝: 수정 + 삭제 모두 허용
 */
export function ZoningListItem({ zone, onEdit, onDelete }: ZoningListItemProps) {
  const isAutoZone = zone.source === 'auto'
  const modeLabel = isAutoZone ? '자동' : '수동'
  const modeClassName = isAutoZone
    ? 'bg-[#EEE9FF] text-[#5D4AD8]'
    : 'bg-[#EAF0FF] text-[#3B45B3]'
  ...
  if (isAutoZone) {
    return (
      <button
        type="button"
        onClick={() => onEdit(zone)}
        className="w-full rounded-lg border border-transparent bg-[#F8F9FD] px-3 py-2.5 text-left transition-colors hover:border-[#D9DEF0]"
      >
        <ZoneSummary zone={zone} modeLabel={modeLabel} modeClassName={modeClassName} />
      </button>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <ZoneSummaryLeft zone={zone} />
        <div className="flex shrink-0 items-center gap-2">
          <span className={`rounded-md px-2 py-1 text-[10px] font-black ${modeClassName}`}>{modeLabel}</span>
          <button
            type="button"
            onClick={(event) => {
...
function ZoneSummary({ zone, modeLabel, modeClassName }: ZoneSummaryProps) {
  return (
    <div className="flex items-center justify-between gap-2">
      <ZoneSummaryLeft zone={zone} />
      <span className={`rounded-md px-2 py-1 text-[10px] font-black ${modeClassName}`}>{modeLabel}</span>
