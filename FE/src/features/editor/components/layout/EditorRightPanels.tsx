...
import { getRightDockWidthClass, getVisiblePanelKeys } from './right-panels/rightPanelVisibility'
...
export function EditorRightPanels(props: EditorRightPanelsProps) {
  const { mode, isCollaborationMode, panelOpenState } = props
  const visiblePanelKeys = getVisiblePanelKeys(mode)
  const rightDockWidthClass = getRightDockWidthClass(visiblePanelKeys, panelOpenState)

  if (isCollaborationMode && mode !== 'bubble') {
    return <EditorRightPanelsCollaborationDock {...props} />
  }

  return <EditorRightPanelsWorkspaceDock rightDockWidthClass={rightDockWidthClass} props={props} />
}
