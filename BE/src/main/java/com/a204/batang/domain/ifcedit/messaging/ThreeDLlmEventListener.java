...
public class ThreeDLlmEventListener {
    ...
    @Transactional
    @RabbitListener(queues = RabbitMqConfig.THREE_D_LLM_DLQ)
    public void consumeDlq(Message message) {
        ...
        try {
            ...
            log.error("3D LLM DLQ message parse failed. headers={}", message.getMessageProperties().getHeaders(), exception);
            return;
        }

        if (commandMessage.jobId() == null || commandMessage.jobStepId() == null) {
            log.error("3D LLM DLQ message missing identifiers. jobId={}, jobStepId={}",
                    commandMessage.jobId(), commandMessage.jobStepId());
            return;
        }

        IfcEditJob job = ifcEditJobRepository.findByJobIdAndJobType(
                commandMessage.jobId(), JOB_TYPE_THREE_D_TO_IFC_EDIT
        ).orElse(null);
        ...
        if (job == null || step == null) {
            log.warn("3D LLM DLQ message ignored because job state not found. jobId={}, jobStepId={}",
                    commandMessage.jobId(), commandMessage.jobStepId());
            return;
        }
        if (job.isTerminal() || step.isTerminal()) {
            log.info("3D LLM DLQ message ignored because state is already terminal. jobId={}, jobStepId={}",
                    commandMessage.jobId(), commandMessage.jobStepId());
            return;
        }
        ...
        JsonNode outputPayload = buildDlqPayload(RabbitMqConfig.THREE_D_LLM_DLQ);
        ...
        log.warn("3D LLM DLQ message marked job failed. projectId={}, jobId={}, jobStepId={}",
    ...
    private void handleStarted(IfcEditEventMessage event) {
        ...
        if (job.isTerminal() || step.isTerminal()) {
            log.info("3D LLM started 이벤트를 무시합니다. jobId={}, reason=terminal-state", event.jobId());
            return;
        }
        ...
        publishStatusEvent(event.projectId(), SSE_IFC_EDIT_STARTED, new IfcEditStatusSseResponse(
                ...
                null, job.getJobType(), "RUNNING", progress, "3D LLM 작업이 시작되었습니다."
        ));
    }
    ...
    private void handleProgress(IfcEditEventMessage event) {
        ...
        if (job.isTerminal() || step.isTerminal()) {
            log.info("3D LLM progress 이벤트를 무시합니다. jobId={}, reason=terminal-state", event.jobId());
            return;
        }
        ...
        publishStatusEvent(event.projectId(), SSE_IFC_EDIT_PROGRESS, new IfcEditStatusSseResponse(
                ...
                null, job.getJobType(), "RUNNING", progress, "3D LLM 작업이 진행 중입니다."
        ));
    }
    ...
    private void handleCompleted(IfcEditEventMessage event) {
        ...
        if (job.isTerminal() || step1.isTerminal()) {
            log.info("3D LLM completed 이벤트를 무시합니다. jobId={}, reason=terminal-state", event.jobId());
            return;
        }
    ...
    private void handleFailed(IfcEditEventMessage event) {
        ...
        if (job.isTerminal() || step1.isTerminal()) {
            log.info("3D LLM failed 이벤트를 무시합니다. jobId={}, reason=terminal-state", event.jobId());
            return;
        }

        String errorCode = event.error() != null && event.error().code() != null
                ? event.error().code() : "THREE_D_LLM_FAILED";
        String errorMessage = event.error() != null && event.error().message() != null
                ? event.error().message() : "3D LLM 작업이 실패했습니다.";
    ...
    private IfcEditJob findJob(IfcEditEventMessage event) {
        return ifcEditJobRepository.findByJobIdAndJobType(event.jobId(), JOB_TYPE_THREE_D_TO_IFC_EDIT)
                .orElseThrow(() -> new CustomException(ErrorCode.IFC_EDIT_JOB_NOT_FOUND));
    }
