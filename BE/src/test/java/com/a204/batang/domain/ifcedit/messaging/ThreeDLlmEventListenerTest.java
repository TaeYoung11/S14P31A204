...
class ThreeDLlmEventListenerTest {
    ...
    void handleStarted_updatesJobAndStepToRunning() {
        IfcEditEventMessage event = buildEvent(EVENT_THREE_D_LLM_STARTED, null, null, 0.2);
    ...
    private IfcEditEventMessage completedEvent(String editPlanUrl) {
        return buildEvent(EVENT_THREE_D_LLM_COMPLETED, Map.of("storage_url", editPlanUrl), null, 1.0);
    }

    private IfcEditEventMessage failedEvent() {
        return buildEvent(
                EVENT_THREE_D_LLM_FAILED,
                null,
                new IfcEditWorkerError("THREE_D_LLM_FAILED", "3D LLM 실패", false, true, null),
                0.0
        );
    }
}
