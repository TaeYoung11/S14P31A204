"""Publish manual test commands to dockerized workers.

Usage:
  - Start the relevant RabbitMQ and worker containers first.
  - This is a manual verification script, not a pytest test.
  - It may publish commands that intentionally fail if referenced artifacts are missing.
"""
...
def main() -> None:
    # RabbitMQ settings only; worker_type is used to load runtime config.
    ...
    command_3d = CommandMessage(
        ...
        input=CommandInputRef(sourceIfcStorageUrl="s3://batang-artifacts/test/model.ifc"),
        ...
        payload=ThreeDLlmCommandPayload(
            userInstruction="거실에 창문을 크게 만들어줘.",
