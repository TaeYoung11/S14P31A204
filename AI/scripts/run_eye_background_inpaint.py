"""Run EYE background/yard inpaint smoke using local ground-neighborhood masks.

This script keeps the first-pass styled house image, then inpaints only the
local non-building ground/background target masks generated from depth.

Examples:
    uv run python scripts/run_eye_background_inpaint.py
"""
...
DEFAULT_MASK_DIR = (
    ROOT
    / "outputs"
    / "ifc2img_eye_ground_neighborhood_target_preview1"
    / "AC20-FZK-Haus"
)
DEFAULT_OUTPUT_DIR = (
    ROOT
    / "outputs"
    / "ifc2img_eye_background_inpaint_smoke1"
    / "AC20-FZK-Haus"
)
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run EYE background/yard inpaint smoke."
    )
...
def _make_contact_sheet(rows: list[tuple[str, Image.Image, Image.Image, Image.Image]]) -> Image.Image:
    ...
    for index, header in enumerate(("input", "mask", "inpaint")):
...
def run(args: argparse.Namespace) -> list[Path]:
    ...
    for index, view in enumerate(args.views):
        ...
        mask_path = mask_dir / f"background_inpaint_target_{view}.png"
        ...
        result_path = (
            output_dir
            / f"eye_background_inpaint_{view}_{args.preset}_s{int(round(args.strength * 100)):03d}_seed{args.seed + index}.png"
        )
        output.save(result_path, format="PNG")
        saved.append(result_path)
        rows.append((view, source, mask, output))

    sheet_path = output_dir / "compare_eye_background_inpaint_smoke.png"
