"""Runtime bootstrap for the IFC generate worker process."""
...
from ai_layout_import.runtime import IfcGenerateWorker, StorageClient
...
StorageFactory = Callable[[S3Settings], StorageClient]
...
def run_ifc_generate_worker(
    ...
    *,
    ...
    if runtime_settings.worker_type != WORKER_TYPE:
        raise ValueError(
            f"IFC generate worker requires WORKER_TYPE={WORKER_TYPE!r}, "
    ...
    try:
        with publisher_factory(runtime_settings.rabbitmq) as publisher:
            storage_client = storage_factory(runtime_settings.s3)
            ...
            logger.info(
                "ifc_generate_worker_starting",
                queue="batang.ifc-generate.command.queue",
                once=once,
            )
            consumer.run()
            logger.info("ifc_generate_worker_stopped", once=once)
            return 0
    finally:
        health_server.stop()
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run the IFC generate worker.")
    parser.add_argument(
        "--once",
        action="store_true",
    ...
    return run_ifc_generate_worker(once=args.once)


__all__ = ["WORKER_TYPE", "build_settings", "main", "run_ifc_generate_worker"]
